...
contract MCCXToken is KIP7Pausable {
    ...
    constructor(
        ...
        address _exchangeMaster, 
        address _roleAccount, 
        address _adminAccount
    ) KIP7(name, symbol) {     
        _mint(_exchangeMaster, initialSupply.mul(10 ** uint256(decimals())));
        
        _setupRole(DEFAULT_ADMIN_ROLE, _adminAccount);
        _setupRole(PAUSER_ROLE, _roleAccount);
        _setRoleAdmin(PAUSER_ROLE, DEFAULT_ADMIN_ROLE);

        _setupRole(LIMITED_WALLET_MANAGER_ROLE, _roleAccount);
        _setRoleAdmin(LIMITED_WALLET_MANAGER_ROLE, DEFAULT_ADMIN_ROLE);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require((!limitedSenderWallets[from] && !limitedReceiverWallets[to]), "MCCXToken: One or both addresses are limited for transfering");
    }
}
